# file_transfer.py - Mixin for file send/receive logic
...
class FileTransferMixin:
    """Mixin providing file transfer helper methods."""

    def _get_temp_dir(self) -> str:
        """Return a dedicated temporary directory path."""
    ...
    def _send_archive(self, sock, archive_path, dest_dir):
        ...
        logging.debug("Entering _send_archive. cancel=%s dest=%s", self._cancel_transfer.is_set(), dest_dir)
    ...
    def _share_files_thread(self, paths, operation):
        self._cancel_transfer.clear()
        logging.debug("Entering _share_files_thread. Cancel flag: %s", self._cancel_transfer.is_set())
        ...
        if arch_thread.is_alive():
            ...
            self.file_transfer_error.emit("Archiválás időtúllépés (túl nagy fájl?)")
        ...
        try:
            ...
            self._cancel_transfer.clear()
            logging.debug("Exiting _share_files_thread. Network clipboard: %s", self.network_file_clipboard)

    def request_paste(self, dest_dir) -> None:
        self._cancel_transfer.clear()
        logging.debug("request_paste called. role=%s cancel=%s", self.settings['role'], self._cancel_transfer.is_set())
