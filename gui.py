...
from config import APP_NAME, ORG_NAME, DEFAULT_PORT
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__();self.setWindowTitle("KVM Switch Vezérlőpult v7");central_widget=QWidget();self.setCentralWidget(central_widget);main_layout=QVBoxLayout(central_widget);role_box=QGroupBox("Szerepkör");role_layout=QVBoxLayout();self.radio_ado=QRadioButton("Adó (ez a gép irányít)");self.radio_vevo=QRadioButton("Vevő (ezt a gépet irányítják)");role_layout.addWidget(self.radio_ado);role_layout.addWidget(self.radio_vevo);role_box.setLayout(role_layout);main_layout.addWidget(role_box);network_box=QGroupBox("Hálózati Beállítások");network_layout=QGridLayout();network_layout.addWidget(QLabel("Port:"),0,0);self.port=QLineEdit();network_layout.addWidget(self.port,0,1);network_box.setLayout(network_layout);main_layout.addWidget(network_box);monitor_box=QGroupBox("Monitor Kódok (Adó módban)");monitor_layout=QGridLayout();monitor_layout.addWidget(QLabel("Hoszt kód:"),0,0);self.host_code=QLineEdit();monitor_layout.addWidget(self.host_code,0,1);monitor_layout.addWidget(QLabel("Kliens kód:"),1,0);self.client_code=QLineEdit();monitor_layout.addWidget(self.client_code,1,1);monitor_box.setLayout(monitor_layout);main_layout.addWidget(monitor_box);other_box=QGroupBox("Egyéb Beállítások");other_layout=QGridLayout();other_layout.addWidget(QLabel("Gyorsbillentyű:"),0,0);self.hotkey_label=QLabel("Laptop: Ctrl + Numpad 0 | ElitDesk: Ctrl + Numpad 1");other_layout.addWidget(self.hotkey_label,0,1);self.autostart_check=QCheckBox("Automatikus indulás a Windows-szal");other_layout.addWidget(self.autostart_check,1,0,1,2);other_box.setLayout(other_layout);main_layout.addWidget(other_box);self.start_button=QPushButton("KVM Szolgáltatás Indítása");self.start_button.clicked.connect(self.toggle_kvm_service);main_layout.addWidget(self.start_button);self.status_label=QLabel("Állapot: Inaktív");main_layout.addWidget(self.status_label);self.kvm_thread=None;self.kvm_worker=None;self.init_tray_icon();self.load_settings()
    ...
    def on_service_stopped(self):
        if self.kvm_thread:self.kvm_thread.quit();self.kvm_thread.wait()
    ...
    def load_settings(self):
        ...
        self.radio_ado.setChecked(is_ado); self.radio_vevo.setChecked(not is_ado)
        self.port.setText(settings.value("network/port", str(DEFAULT_PORT)))
        self.host_code.setText(settings.value("monitor/host_code", "17")); self.client_code.setText(settings.value("monitor/client_code", "18"))
        self.autostart_check.setChecked(settings.value("other/autostart", False, type=bool))
        self.radio_ado.toggled.connect(self.save_settings); self.port.textChanged.connect(self.save_settings)
        self.host_code.textChanged.connect(self.save_settings); self.client_code.textChanged.connect(self.save_settings)
    ...
    def on_status_update(self, message): self.status_label.setText(message);logging.info(f"GUI Status Update: {message}")
    ...
    def quit_application(self):
        logging.info("Kilépés menüpont kiválasztva. Program leállítása.");self.stop_kvm_service();time.sleep(0.2);QApplication.instance().quit()
